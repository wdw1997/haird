...
async function getAuthedClient(stylistId) {
  ...
  if (error) {
    // This used to be swallowed silently, which is exactly why the last
    // several rounds of debugging had no evidence to go on. Whatever shows
    // up here now is the real reason getAuthedClient is bailing out.
    console.error('decrypt_stylist_token RPC failed for stylist', stylistId, ':', JSON.stringify(error))
    return null
  }
  if (!refreshToken) {
    console.error('decrypt_stylist_token returned an empty token for stylist', stylistId)
    return null
  }
